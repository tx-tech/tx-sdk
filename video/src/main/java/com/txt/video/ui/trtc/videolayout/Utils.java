...
public class Utils {
    ...
    public static ArrayList<RelativeLayout.LayoutParams> initFloatParamList(Context context, int layoutWidth, int layoutHeight) {
        ...
        // 底部最大的布局
        int i1 = layoutHeight / 10;
        int subWidth = layoutWidth / 4;
...
//        final int subWidth = Utils.dip2px(context, 100);
        final int subHeight = Utils.dip2px(context, 140);
        ...
        for (int i = 0; i < 5; i++) {
            if (i == 0) {
                ...
                layoutParams.leftMargin = layoutWidth - subWidth * (i);
