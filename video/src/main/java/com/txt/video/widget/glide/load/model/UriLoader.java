 ...
 * remote {@link Uri}s to a wrapped {@link com.txt.video.widget.glide.load.model.ModelLoader} that handles
 * {@link GlideUrl}s.
 *
...
public abstract class UriLoader<T> implements ModelLoader<Uri, T> {
    ...
    public final DataFetcher<T> getResourceFetcher(Uri model, int width, int height) {
        ...
        if (isLocalUri(scheme)) {
            if (com.txt.video.widget.glide.load.model.AssetUriParser.isAssetUri(model)) {
                String path = com.txt.video.widget.glide.load.model.AssetUriParser.toAssetPath(model);
